...
class SelectionTile(sw.Tile):
    def __init__(self, aoi_model, model, viz_tile, export_tile, **kwargs):
        ...
        # widgets
        w_time_title = v.Html(
            tag="H3", class_="mt-3", children=[cm.selection.time_range]
        )
        self.start_picker = cw.DatePicker(label=cm.selection.start)
        self.end_picker = cw.DatePicker(label=cm.selection.end)
        w_time_range = v.Layout(row=True, children=[self.start_picker, self.end_picker])

        w_collection_title = v.Html(
            tag="H3", class_="mt-3", children=[cm.selection.collection]
        )
        self.sensors = v.Select(
            label=cm.selection.sensor,
            items=pm.sensors,
            v_model=None,
            chips=True,
            multiple=True,
        )
        self.t2 = v.Switch(class_="ml-5", label=cm.selection.t2, v_model=False)
        self.sr = v.Switch(class_="ml-5", label=cm.selection.sr, v_model=False)
        ...
        super().__init__(
            ...
            title=cm.selection.title,  # the Title will be displayed on the top of the tile
            inputs=[
                w_time_title,
                w_time_range,
                w_collection_title,
    ...
    def _on_run(self, widget, data, event):
        ...
        if not self.alert.check_input(self.aoi_model.name, cm.process.no_aoi):
            return
        if not self.alert.check_input(self.model.sensors, cm.process.no_sensors):
            return
        if not self.alert.check_input(self.model.start, cm.process.no_date):
            return
        if not self.alert.check_input(self.model.end, cm.process.no_date):
            return
        ...
        if not dt.strptime(self.model.start, d_format) < dt.strptime(
            ...
            self.alert.add_msg(cm.process.no_order, "error")
            return
